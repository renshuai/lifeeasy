...
Page({
  ...
  getData(isFromPullDown, isFromBottomCallback) {
    ...
    if (this.data.list.length > 0) {
      db.collection('sale').limit(app.globalData.limit).skip(this.data.list.length).get().then(response => {
        wx.hideLoading();
        let data = response.data;
        this.handleData(data, isFromBottomCallback);
      })
    } else {
      db.collection('sale').limit(app.globalData.limit).get().then(response => {
        wx.hideLoading();
        let data = response.data;
