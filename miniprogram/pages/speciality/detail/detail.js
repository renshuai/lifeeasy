...
Page({
  ...
  getDetail() {
    ...
    db.collection('speciality').doc(this.data.id).get().then(response => {
      wx.hideLoading();
      this.setData({
        data: response.data,
  ...
  edit() {
    wx.navigateTo({
      url: '/pages/publish/speciality/speciality?id=' + this.data.id
    })
  },
  delete() {
    wx.showModal({
  ...
  deleteHandler() {
    ...
    const env = app.globalData.env;
    const db = wx.cloud.database({ env: env });
    db.collection('speciality').doc(this.data.id).remove().then(_ => {
      wx.hideLoading();
      wx.showToast({
        title: '删除成功'
      });
