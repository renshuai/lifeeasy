...
Page({
  ...
  onLoad(e) {
    ...
    if (e.id) {
      ...
      db.collection('speciality').doc(e.id).get().then(response => {
        wx.hideLoading();
  ...
  publish() {
    if (!this.data.speciality || !this.data.phone) {
      wx.showToast({
        title: '必填项不能为空'
      })
      return;
    ...
    if (this.data.id) {
      ...
      db.collection('speciality').doc(this.data.id).update({
        data: {
          type: +this.data.type,
